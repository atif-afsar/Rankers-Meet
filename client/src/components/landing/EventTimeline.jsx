      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:grid md:grid-cols-5 gap-6 sm:gap-6 relative z-10">
          {schedule.map((item, idx) => {
            ...
            const phaseLabel = item.step || `Part 0${idx + 1}`;
            ...
            return (
              ...
              >
                {/* Phase Indicator Node (Sequence-based, clean and time-independent) */}
                <div className="relative flex-shrink-0 mb-0 md:mb-5 z-10">
                  ...
                  {/* Top Phase Pill */}
                  <div className="hidden md:block absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap text-[11px] font-black text-[#D91F2B] uppercase tracking-wider bg-[#FFF7F7] px-2.5 py-0.5 rounded-full border border-[#F0D5D7]">
                    {item.time || phaseLabel}
                  </div>
                </div>
                ...
                <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[#F0D5D7] shadow-sm group-hover:shadow-card transition-all w-full flex-1">
                  ...
                  <div className="md:hidden inline-flex items-center space-x-1 text-[11px] font-bold text-[#D91F2B] bg-[#FFF7F7] px-2.5 py-0.5 rounded-full border border-[#F0D5D7] mb-2">
                    <span>{item.time || phaseLabel}</span>
                  </div>
